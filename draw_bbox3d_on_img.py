...
# label
# 2d bbox in image coord.
xmin, ymin, xmax, ymax = 712.40, 143.00, 810.73, 307.92
bbox2d = [xmin, ymin, xmax, ymax]
height, width, length = 1.89, 0.48, 1.20  # 3d object size in lidar coord.
t = [1.84, 1.47, 8.41]  # location (x,y,z) in rectified camera coord.  ??
ry = 0.01  # yaw angle in rectified camera coord. [-pi, pi]


# project rect 3D point to image
...
s = np.sin(ry)
c = np.cos(ry)
R_obj_to_rect = np.array([[c, 0, s],
                          [0, 1, 0],
                          [-s, 0, c]])
x = np.dot(R_obj_to_rect, x)  # 3x8
x[0, :] = x[0, :] + t[0]
x[1, :] = x[1, :] + t[1]
x[2, :] = x[2, :] + t[2]
...
# rect -> img
x = calib.project_rect_to_img(x)  # 8x2
